    ...
    if settings["button"]:
        if URL_MODE == False:
            ...
            if msg.message.chat.id in LAZY_GROUPS:
                ...
                btn = [
                [
                    InlineKeyboardButton(text=f"[{get_size(file.file_size)}] {file.file_name}", url=await get_shortlink(f"https://telegram.dog/{temp.U_NAME}?start={pre}#{file.file_id}")),
                ]
                for file in files
                ]
    else:
        if URL_MODE == False:
            ...
            if msg.message.chat.id in LAZY_GROUPS:
                ...
                [
                    InlineKeyboardButton(
                        text=f"[{get_size(file.file_size)}] {file.file_name}", 
                        url=await get_shortlink(f"https://telegram.dog/{temp.U_NAME}?start={pre}#{file.file_id}")
                    ),
                    InlineKeyboardButton(
                        text=f"[{get_size(file.file_size)}] {file.file_name}", 
                        url=await get_shortlink(f"https://telegram.dog/{temp.U_NAME}?start={pre}#{file.file_id}")
